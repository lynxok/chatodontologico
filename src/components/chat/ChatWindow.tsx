...
import { Send, Paperclip, MoreVertical, FileText, Image as ImageIcon, Download, X } from 'lucide-react';
...
export const ChatWindow: React.FC<ChatWindowProps> = ({ currentUser, target }) => {
  ...
  const targetId = isBroadcast ? null : (target?.username ?? null);
  
  // Use current user's personalized replies or defaults
  ...
  const markAsRead = async () => {
    // Intentar marcar como leído usando tanto el ID como el username por seguridad
    const identifier = target?.id || target?.username;
    if (isBroadcast || !identifier || !currentUser.username) return;

    console.log('Marcando como leído:', identifier);

    await supabase
      .from('messages')
      .update({ read_at: new Date().toISOString() })
      .or(`sender_id.eq.${identifier},sender_id.eq.${target?.username}`)
  ...
  const fetchMessages = async () => {
    ...
    if (isBroadcast) {
      ...
      query = query.or(
        `and(sender_id.eq.${currentUser.username},recipient_id.eq.${targetId}),and(sender_id.eq.${targetId},recipient_id.eq.${currentUser.username})`
      );
    }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Register attachment for auto-delete (60 days)
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#f0f4f5', overflow: 'hidden', minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', height: '70px', minHeight: '70px', backgroundColor: 'white', borderBottom: '1px solid #e8eef0', flexShrink: 0 }}>
        <div>
          <div style={{ fontWeight: 800, fontSize: '17px', color: '#1A3A3A' }}>{isBroadcast ? 'Comunicación Interna' : targetName}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '3px' }}>
            <span style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#22c55e', display: 'inline-block' }}></span>
            <span style={{ fontSize: '12px', color: '#22c55e', fontWeight: 600 }}>{isBroadcast ? 'Todos los consultorios' : `Conectado`}</span>
          </div>
        </div>
          ...
          return (
            <div key={msg.id} style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start', maxWidth: '70%' }}>
                <div style={{ 
                  padding: msg.file_url && isImage ? '8px' : '12px 16px', 
                  borderRadius: isMe ? '20px 20px 4px 20px' : '20px 20px 20px 4px',
                  backgroundColor: isMe ? '#0ABAB5' : 'white',
                  ...
                  {msg.file_url ? (
                    isImage ? (
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '5px' }}>
                        <div style={{ p: '10px', borderRadius: '10px', backgroundColor: isMe ? 'rgba(255,255,255,0.2)' : '#f0f7f7' }}>
                          <FileText size={24} />
                        </div>
      ...
      <div style={{ padding: '10px 24px', display: 'flex', gap: '8px', flexWrap: 'wrap', backgroundColor: 'transparent' }}>
        {quickReplies.map((reply) => (
          <button
            ...
            style={{ padding: '8px 16px', borderRadius: '12px', border: '1.5px solid #d1d5db', backgroundColor: 'white', color: '#374151', fontSize: '12px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit' }}
            onMouseOver={e => { e.currentTarget.style.borderColor = '#0ABAB5'; e.currentTarget.style.color = '#0ABAB5'; }}
      ...
      <div style={{ backgroundColor: 'white', padding: '15px 24px 25px', borderTop: '1px solid #e8eef0' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: '#f8fafb', borderRadius: '18px', padding: '6px 6px 6px 16px', border: '1.5px solid #eef2f2' }}>
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: '8px' }}>
          ...
          <input
            ...
            style={{ flex: 1, border: 'none', outline: 'none', backgroundColor: 'transparent', fontSize: '14px', color: '#1A3A3A', fontWeight: '600', fontFamily: 'inherit' }}
          />

          <button type="submit" disabled={!inputValue.trim() || isUploading} style={{ width: '45px', height: '45px', borderRadius: '14px', border: 'none', backgroundColor: inputValue.trim() ? '#0ABAB5' : '#eef2f2', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Send size={18} />
          </button>
        </form>
      </div>
